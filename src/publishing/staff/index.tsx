...
export default function StaffPublishingGallery() {
  ...
  const categories = useMemo(
    () => ['전체', ...Array.from(new Set(staffPublishingScreens.map((screen) => screen.category)))],
    []
  );

  const filtered = useMemo(() => {
    return staffPublishingScreens.filter((screen) => {
  ...
  const totalCount = staffPublishingScreens.length;
  const modalCount = staffPublishingScreens.filter((screen) => screen.state === '모달').length;
  const tabCount = staffPublishingScreens.filter((screen) => screen.state === '탭').length;

  return (
    <div className="h-[100dvh] overflow-y-auto overscroll-y-contain bg-[radial-gradient(circle_at_top_left,#e2e8f0_0%,#f1f5f9_28%,#f8fafc_62%,#ffffff_100%)]">
          ...
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-600 sm:text-base">
                실제 스태프 라우트를 그대로 iframe으로 묶은 퍼블리싱 갤러리입니다. 별도 복제 화면이 아니라
                <code className="rounded bg-slate-100 px-1.5 py-0.5 text-[0.95em]">preview=1&amp;role=staff</code> 모드의 실제 페이지를 그대로 보여줍니다.
              </p>
            </div>
