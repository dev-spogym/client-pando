...
import { trainerPublishingScreens } from './screens';
import { PhoneFrame, PreviewCard } from '@/publishing/member/ui';

const stateFilters = ['전체', '기본', '탭', '모달', '완료', '에러'] as const;

export default function TrainerPublishingGallery() {
  const [category, setCategory] = useState('전체');
  const [state, setState] = useState<(typeof stateFilters)[number]>('전체');
  const [query, setQuery] = useState('');

  const categories = useMemo(
    () => ['전체', ...Array.from(new Set(trainerPublishingScreens.map((screen) => screen.category)))],
    []
  );

  const filtered = useMemo(() => {
    return trainerPublishingScreens.filter((screen) => {
  ...
  const totalCount = trainerPublishingScreens.length;
  const modalCount = trainerPublishingScreens.filter((screen) => screen.state === '모달').length;
  const tabCount = trainerPublishingScreens.filter((screen) => screen.state === '탭').length;

  return (
    <div className="h-[100dvh] overflow-y-auto overscroll-y-contain bg-[radial-gradient(circle_at_top_left,#ccfbf1_0%,#f0fdfa_28%,#f8fafc_62%,#ffffff_100%)]">
          ...
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-white">
                ...
                trainer publishing folder
              </div>
              <h1 className="mt-4 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
                트레이너 퍼블리싱 갤러리
              </h1>
              <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-600 sm:text-base">
                실제 트레이너 라우트를 그대로 iframe으로 묶은 퍼블리싱 갤러리입니다. 별도 복제 화면이 아니라
                <code className="rounded bg-slate-100 px-1.5 py-0.5 text-[0.95em]">preview=1&amp;role=trainer</code> 모드의 실제 페이지를 그대로 보여줍니다.
              </p>
            </div>
