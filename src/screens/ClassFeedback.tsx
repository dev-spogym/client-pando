...
import { Button, Card, Chip, Badge, PageHeader } from '@/components/ui';
...
export default function ClassFeedback() {
  ...
  return (
    <div className="min-h-screen bg-surface-secondary page-with-action">
      <PageHeader title="수업 후기" showBack />

      <div className="px-4 py-4 space-y-4 pb-28">
        <Card variant="soft" padding="lg">
          <p className="text-caption text-content-tertiary">완료 수업</p>
          <h2 className="text-h3 text-content mt-1">{classInfo?.title || '수업 정보 확인 중'}</h2>
          <p className="text-body-sm text-content-secondary mt-2">{classInfo?.staffName || '-'} 강사</p>
        </Card>

        {existing ? (
          <Card variant="soft" padding="lg">
            <p className="text-body-sm font-semibold">이미 후기 작성이 완료되었습니다.</p>
            ...
            {existing.comment && (
              <p className="mt-4 text-body-sm text-content-secondary leading-relaxed">{existing.comment}</p>
            )}
          </Card>
        ) : (
          <>
            <Card variant="soft" padding="lg">
              <h3 className="text-body font-semibold mb-3">별점</h3>
            ...
            <Card variant="soft" padding="lg">
              <h3 className="text-body font-semibold mb-3">빠른 태그</h3>
            ...
            <Card variant="soft" padding="lg">
              <h3 className="text-body font-semibold mb-3">한줄 후기</h3>
              <textarea
                value={comment}
              ...
              />
              <p className="mt-2 text-caption text-content-tertiary text-right">{comment.length}/200</p>
            </Card>

            <Card variant="soft" padding="lg">
              <h3 className="text-body font-semibold mb-3">추천 의향 점수</h3>
              <div className="grid grid-cols-6 gap-2">
                {Array.from({ length: 11 }).map((_, index) => (
                  <button
                    ...
                    className={cn(
                      'py-2 rounded-lg text-body-sm font-medium',
      ...
      {!existing && (
        <div className="bottom-action-bar">
          <div className="max-w-lg mx-auto">
            <Button variant="primary" size="xl" fullWidth onClick={handleSubmit}>
              후기 제출
            </Button>
          </div>
        </div>
